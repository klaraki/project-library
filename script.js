...
const filterRecipes = () => {
 ...
 if (value === "all") {
   loadRecipes(recipes);
    ...
    // Otherwise, filter recipes based on country and load the filtered list.
    const filteredList = recipes.filter((recipe) => recipe.cuisineType === value);
...
const sortRecipes = () => {
 ...
 const sortingValue = sortingDropDown.value;


 if (sortingValue === "Sort by time") {
   loadRecipes(recipes);

  } else if (sortingValue === "lowest") {
    // Sort the recipes by lowest time and load the filtered list. 

    const sortedByLowest = recipes.sort((recipeOne, recipeTwo) => (recipeOne.totalTime > recipeTwo.totalTime) ? 1 : (recipeOne.totalTime < recipeTwo.totalTime) ? -1 : 0);

    loadRecipes(sortedByLowest);

  } else if (sortingValue === "highest") {
    // Sort the recipes by highest time and load the filtered list. 

    const sortedByHighest = recipes.sort((recipeOne, recipeTwo) => (recipeOne.totalTime < recipeTwo.totalTime) ? 1 : (recipeOne.totalTime > recipeTwo.totalTime) ? -1 : 0);

    loadRecipes(sortedByHighest);
